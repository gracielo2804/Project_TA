...
internal object PDFUtilityShoppingList {
    private val TAG = PDFUtilityShoppingList::class.java.simpleName
...
//            logoTable.addCell(logoCell)
    ...
    private fun createDataTable(dataTable: List<Array<String>>): PdfPTable {
        ...
        table1.setWidths(floatArrayOf(2f,3f,3f))
        table1.headerRows = 1
        table1.defaultCell.verticalAlignment = Element.ALIGN_CENTER
        table1.defaultCell.horizontalAlignment = Element.ALIGN_CENTER
        var cell: PdfPCell
        run {
            cell = PdfPCell(Phrase("Image", FONT_COLUMN))
            cell.horizontalAlignment = Element.ALIGN_CENTER
            cell.verticalAlignment = Element.ALIGN_MIDDLE
            cell.setPadding(4f)
            table1.addCell(cell)

            cell = PdfPCell(Phrase("Recipe Name", FONT_COLUMN))
            cell.horizontalAlignment = Element.ALIGN_CENTER
            cell.verticalAlignment = Element.ALIGN_MIDDLE
            cell.setPadding(4f)
            table1.addCell(cell)

            cell = PdfPCell(Phrase("Ingredients", FONT_COLUMN))
            cell.horizontalAlignment = Element.ALIGN_CENTER
            cell.verticalAlignment = Element.ALIGN_MIDDLE
            cell.setPadding(4f)
            table1.addCell(cell)
        ...
        for (i in 0 until size) {
            ...
            cell = PdfPCell()
            cell.image = getImage(listImageByteArray[i],false)
