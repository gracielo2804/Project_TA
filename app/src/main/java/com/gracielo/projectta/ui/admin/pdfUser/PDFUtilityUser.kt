package com.gracielo.projectta.ui.admin.pdfUser
...
import com.gracielo.projectta.ui.admin.pdfMembership.PageNumeration
...
internal object PDFUtilityUser {
    private val TAG = PDFUtilityUser::class.java.simpleName
    ...
    private var imagePieChart:ByteArray?=null
    private var imageBarChart:ByteArray?=null
    ...
    @Throws(Exception::class)
    fun setImagePie(byteArray: ByteArray){
        imagePieChart=byteArray
    }

    @Throws(Exception::class)
    fun setImageBar(byteArray: ByteArray){
        imageBarChart=byteArray
    }
    ...
    fun createPdf(
        ...
        document.add(getImage(imagePieChart!!,false))
        document.add(getImage(imageBarChart!!,false))
...
//        document.addHeader("DEVELOPER", "RAVEESH G S")
    ...
    private fun addHeader(mContext: Context, document: Document) {
        ...
        run {
            ...
            temp = Paragraph("Application Users Report", FONT_SUBTITLE)
            temp.alignment = Element.ALIGN_CENTER
            cell.addElement(temp)
    ...
    private fun createDataTable(dataTable: List<Array<String>>): PdfPTable {
        ...
        table1.widthPercentage = 100f
        table1.setWidths(floatArrayOf(2f,2f,2f,1f,1f,1f))
        table1.headerRows = 1
        table1.defaultCell.verticalAlignment = Element.ALIGN_CENTER
        table1.defaultCell.horizontalAlignment = Element.ALIGN_CENTER
        var cell: PdfPCell
        run {
            cell = PdfPCell(Phrase("Username", FONT_COLUMN))
            cell.horizontalAlignment = Element.ALIGN_CENTER
            cell.verticalAlignment = Element.ALIGN_MIDDLE
            cell.setPadding(4f)
            table1.addCell(cell)
            cell = PdfPCell(Phrase("Name", FONT_COLUMN))
            cell.horizontalAlignment = Element.ALIGN_CENTER
            cell.verticalAlignment = Element.ALIGN_MIDDLE
            cell.setPadding(4f)
            table1.addCell(cell)
            ...
            cell = PdfPCell(Phrase("Email Verification", FONT_COLUMN))
            cell.horizontalAlignment = Element.ALIGN_CENTER
            cell.verticalAlignment = Element.ALIGN_MIDDLE
            cell.setPadding(4f)
            table1.addCell(cell)

            cell = PdfPCell(Phrase("Expired Time", FONT_COLUMN))
            cell.horizontalAlignment = Element.ALIGN_CENTER
            cell.verticalAlignment = Element.ALIGN_MIDDLE
            cell.setPadding(4f)
            table1.addCell(cell)

            cell = PdfPCell(Phrase("Last Login", FONT_COLUMN))
            cell.horizontalAlignment = Element.ALIGN_CENTER
            cell.verticalAlignment = Element.ALIGN_MIDDLE
            cell.setPadding(4f)
            table1.addCell(cell)
        ...
        for (i in 0 until size) {
            ...
            cell = PdfPCell(Phrase(temp[2], FONT_CELL))
            cell.horizontalAlignment = Element.ALIGN_CENTER
            cell.verticalAlignment = Element.ALIGN_MIDDLE
            cell.paddingLeft = left_right_Padding
            cell.paddingRight = left_right_Padding
