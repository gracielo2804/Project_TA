package com.gracielo.projectta.ui.admin.pdfIngredientsRecipe
...
internal object PDFUtilityIngredientsRecipe {
    private val TAG = PDFUtilityIngredientsRecipe::class.java.simpleName
    ...
    fun setType(type:String){
       PDFUtilityIngredientsRecipe.type =type
    }
    @Throws(Exception::class)
...
//        document.addHeader("DEVELOPER", "RAVEESH G S")
    ...
    private fun addHeader(mContext: Context, document: Document) {
        ...
        run {
            ...
            if(type =="Ingredients")temp = Paragraph("Recipe Search Count Report", FONT_SUBTITLE)
            else if(type =="Recipe")temp = Paragraph("Ingredients Report Detail", FONT_SUBTITLE)
            temp.alignment = Element.ALIGN_CENTER
            cell.addElement(temp)
