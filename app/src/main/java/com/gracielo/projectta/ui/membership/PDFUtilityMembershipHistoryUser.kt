...
internal object PDFUtilityMembershipHistoryUser {
    private val TAG = PDFUtilityMembershipHistoryUser::class.java.simpleName
    ...
    private var listImageByteArray = mutableListOf<ByteArray>()
    ...
    @Throws(Exception::class)
    fun setImage(listImage : MutableList<ByteArray>){
        listImageByteArray = listImage
    }
...
//        document.addHeader("DEVELOPER", "RAVEESH G S")
    ...
    private fun addHeader(mContext: Context, document: Document) {
        ...
        run {
            ...
            temp = Paragraph("Shopping List Report", FONT_SUBTITLE)
            temp.alignment = Element.ALIGN_CENTER
            cell.addElement(temp)
...
//            val drawable = ContextCompat.getDrawable(mContext, R.drawable.logo_apps)
...
//            logoTable.addCell(logoCell)
    ...
    private fun createDataTable(dataTable: List<Array<String>>): PdfPTable {
        ...
        table1.setWidths(floatArrayOf(2f,1f,2f,1f,1f))
        table1.headerRows = 1
        ...
        run {
            ...
            cell = PdfPCell(Phrase("Date", FONT_COLUMN))
            cell.horizontalAlignment = Element.ALIGN_CENTER
            cell.verticalAlignment = Element.ALIGN_MIDDLE
            cell.setPadding(4f)
            table1.addCell(cell)

            cell = PdfPCell(Phrase("Plan", FONT_COLUMN))
            cell.horizontalAlignment = Element.ALIGN_CENTER
            cell.verticalAlignment = Element.ALIGN_MIDDLE
            cell.setPadding(4f)
            table1.addCell(cell)

            cell = PdfPCell(Phrase("Price (IDR)", FONT_COLUMN))
            cell.horizontalAlignment = Element.ALIGN_CENTER
            cell.verticalAlignment = Element.ALIGN_MIDDLE
            cell.setPadding(4f)
            table1.addCell(cell)
